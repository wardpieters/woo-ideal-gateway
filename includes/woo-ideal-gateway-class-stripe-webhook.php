				...
				if($data['type'] == "ideal") {
					...
					if($data['status'] == "chargeable") {
						...
						if($source == $source_stripe) {
							...
							if (is_wp_error($response)) {
							   ...
							   $data = array(
								...
								if($response['paid'] == true) {
									...
									$order->update_status('failed', __('iDEAL payment failed - Error #0003!', 'woo-ideal-gateway')); // order note is optional, if you want to  add a note to order
							...
							$order->update_status('failed', __('iDEAL payment failed - Error #0001!', 'woo-ideal-gateway')); // order note is optional, if you want to  add a note to order
						...
						$order->update_status('failed', __('iDEAL payment failed - Error: #0002', 'woo-ideal-gateway')); // order note is optional, if you want to  add a note to order
